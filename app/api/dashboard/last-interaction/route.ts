...
const logger = getLogger("api/dashboard/last-interaction");
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logger.info(`[${requestId}] Last interaction data retrieved`, {
      from,
      to,
      aiSessions: data.aiSessions,
      humanSessions: data.humanSessions,
    });
    return NextResponse.json(data);
  } catch (err) {
    const error = err instanceof Error ? err : new Error(String(err));
    logger.error(`[${requestId}] Failed to fetch last interaction data`, {
      from,
      to,
      error: error.message,
      stack: error.stack,
